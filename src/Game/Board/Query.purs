module Game.Board.Query
  ( adjacentRelativeEdge
  , buildBoardGraph
  , buildConnectionMap
  , connectedRelativeEdge
  , getBoardEdgePseudoLocation
  , getBoardRelEdge
  , getPortOnEdge
  , isInsideBoard
  , toAbsoluteEdge
...
import Control.Monad.Writer (class MonadWriter, WriterT, execWriterT, runWriter, runWriterT, tell)
...
import Game.Direction (CardinalDirection, allDirections, oppositeDirection)
import Game.Direction as Direction
import Game.Edge (Edge(..), matchEdge)
import Game.Location (Location(..), location)
import Game.Piece (getPort, portMatches, portType)
...
buildBoardGraph = do
  ...
  let toGraphMap inEdge outEdge = M.singleton (relativeEdgeLocation outEdge) (Tuple unit (L.singleton (relativeEdgeLocation inEdge)))
  let connectedGraphMap = M.foldSubmap Nothing Nothing toGraphMap connections

  pieces <- use _pieces
  let unconnectedGraphMap = M.foldSubmap Nothing Nothing (\loc _ -> M.singleton loc (Tuple unit Nil)) pieces
  pure $ G.fromMap (connectedGraphMap <> unconnectedGraphMap)
